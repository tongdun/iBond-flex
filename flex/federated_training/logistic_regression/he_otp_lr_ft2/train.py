    ...
    def _get_encryptor_decryptor(self):
        ...
        if self.sec_param.he_algo == CRYPTO_PAILLIER:
            ...
            raise NotImplementedError(
                f'Encryption algorithm {self.sec_param.he_algo} is not supported.')
...
class HEOTPLRCoord(BaseProtocol, Mixin):
    ...
    def __init__(self, federal_info: dict, sec_param: dict = None):
        ...
        self.chan_guest_coordinator = make_variable_channel('he_otp_lr_ft2_variable_G2C', self.federal_info.guest[0],
                                                            self.federal_info.coordinator)

        self._get_encryptor_decryptor()
        # Step 1
        self.chan_guest_coordinator.send(self.encryptor, tag='encryptor')
        self.chan_host_coordinator.send(self.encryptor, tag='encryptor')
    ...
    def exchange(self, *args, **kwargs) -> None:
        ...
        # Step 5
        enc_padded_grads = self.chan_host_coordinator.recv(tag='enc_padded_grads')
        padded_grads = self.decryptor.decrypt(enc_padded_grads)

        self.chan_host_coordinator.send(padded_grads, tag='padded_grads')


class HEOTPLRGuest(BaseProtocol, Mixin):
    """Guest side protocol
    """
    ...
    def __init__(self, federal_info: dict, sec_param: dict):
        ...
        # Step 1:
        self.encryptor = self.chan_guest_coordinator.recv(tag='encryptor')
    ...
    def exchange(self, theta: np.ndarray, features: np.ndarray, labels: np.ndarray) -> Tuple[np.ndarray, np.ndarray]:
        ...
        # Step 3
        u_2 = self.chan_host_guest.recv(tag='u_2')
        u_1 = theta.dot(features.T)
        u = u_1 + u_2
        h_x = 1 / (1 + np.exp(-u))
        enc_diff_y = self.encryptor.encrypt((labels - h_x))
        self.chan_host_guest.send(enc_diff_y, tag='enc_diff_y')
...
class HEOTPLRHost(BaseProtocol, Mixin):
    """Host side protocol
    """
    ...
    def exchange(self, theta: np.ndarray, features: np.ndarray, *args, **kwargs) -> np.ndarray:
        ...
        self.chan_host_guest.send(u_2, tag='u_2')
        # Step 4
        enc_diff_y = self.chan_host_guest.recv(tag='enc_diff_y')
        batch_size = features.shape[0]
        ...
        self.chan_host_coordinator.send(enc_padded_grads, tag='enc_padded_grads')

        # Step 6
        padded_grads = self.chan_host_coordinator.recv(tag='padded_grads')

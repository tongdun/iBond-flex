...
def make_protocol(protocol_name: str,
    ...
    elif protocol_name == HE_GB_FP:
        from flex.prediction.tree_model.he_gb_fp.predict import NodeFPParty

        all_class = {"guest": NodeFPParty, "host": NodeFPParty}
    ...
    elif protocol_name == OTP_PN_FL:
        from .preprocessing.otp_pn_fl.train import OTPPNCoord, OTPPNGuest, OTPPNHost

        all_class = {"guest": OTPPNGuest, "host": OTPPNHost, 'coordinator': OTPPNCoord}
    ...
    elif protocol_name == RSA_SAL:
        from .sharing.sample_alignment.rsa_sal.align import RSASalGuest, RSASalHost

        all_class = {"guest": RSASalGuest, "host": RSASalHost}
    elif protocol_name == ECDH_SAL:
        from .sharing.sample_alignment.ecdh_sal.align import ECDHSalGuest, ECDHSalHost

        all_class = {"guest": ECDHSalGuest, "host": ECDHSalHost}
    # ================  training  ===================
    # linear_regression
    ...
    # Tree model
    elif protocol_name == HE_GB_FT:
        from .training.tree_model.he_gb_ft.train import HEGBFTGuest, HEGBFTHost

        all_class = {"guest": HEGBFTGuest, "host": HEGBFTHost}

...
def test_sample_filtering():
    ...
    var_chan = make_variable_channel('test_data_exchange', federal_info["federation"]["host"][0],
                                     federal_info["federation"]["guest"][0])
    # 安全参数，输入
    sec_param = []
    ...
    # 将测试数据发送给求交的另一参与方guest，并接收从另一方发送的数据，由于求交是在压缩的filter上进行的，求完交集后需要将对齐数据解压

...
from caffeine.feature.feature_engineering.check_d_type import CheckDTypeHost
...
METHOD_NAME = {
    'hetero': {
        'HeteroIvSelectionHost': HeteroIvSelectionParticipant
    },
    'homo':{
         # TODO
    }
...
class FeatureSelectionHost(FeatureSelectionBase, CheckDTypeHost):
    @ClassMethodAutoLog()
    def __init__(self, meta_params: Dict, context: Context, 
        ...
        self.check = CheckDTypeHost(meta_params)
    ...
    def train(self, train_data: Optional[IBondDataFrame], 
        ...
        """
        Host train selection interface. It is the main entrance
        ...
        for method, config in self.pipeline:
            class_obj = self._class_init(METHOD_NAME, method, "Host")(self._meta_params, config)
    ...
    def predict(self, data: IBondDataFrame) -> IBondDataFrame:
        """
        Host test selection interface. It is the main entrance
        for feature selection, for both hetero and homo senario.
